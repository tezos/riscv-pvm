...
//!
//! 1. **Value-level hashing** (`value_hash/*`): hash standalone byte values of fixed sizes
//!    under the page-tree scheme the AVL stores today, under the length-committed BLAKE3
//!    scheme that is to replace it, and as a raw `blake3::hash` of the same bytes. The raw
//!    figure is the floor: the new value hash is that plus two fixed compressions, so the
//!    gap between it and the page tree is what the migration buys.
//!
