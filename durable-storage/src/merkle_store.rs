...
//!
//! Nodes are content-addressed, so the same node written from any database is the same key here
//! and is stored once. The store sits outside the per-commit checkpoints, so its compaction churn
//! is paid once rather than once per retained commit, and it gives one place from which a node can
//! actually be deleted in future.
//!
//! # One instance per path
//!
//! RocksDB permits a single writer per directory. A repository is reached through [`DirectoryManager`],
//! however, which is cloned freely and may be constructed more than once for the same
...
/// The name the node bodies are reported under when a lookup or a write fails.
///
/// They are in the default column family; this only names it in errors.
const MERKLE_CF: &str = "merkle";
...
impl Stamp {
    ...
    pub fn at(seq: Seq) -> Self {
        Self(seq.raw().checked_add(1).expect("Stamps cannot overflow unless the seq/journal itself was corrupted with too-large `Seq`"))
    }
...
pub struct MerkleStore {
    ...
    /// reopening from racing the close.
...
impl MerkleStore {
    ...
    pub fn get(&self, key: &[u8]) -> Result<rocksdb::DBPinnableSlice<'_>, Error> {
        match found_or_missing(self.db.get_pinned(key), MERKLE_CF, key) {
            Ok(value) => Ok(value),
            ...
            // A key that is not a hash was never a node, so a collection cannot explain its
            // absence - reporting one would put a hash nothing ever wrote into an error callers
            // match on.
            Err(Error::InvalidArgument(InvalidArgumentError::KeyNotFound))
                if self.has_collected() && key.len() == Hash::DIGEST_SIZE =>
            {
                let mut root = [0u8; Hash::DIGEST_SIZE];
                root.copy_from_slice(key);

                Err(OperationalError::NodeCollected {
                    root: Hash::from(root),
                })?
            }
            Err(error) => Err(error),
    ...
    pub fn note_collected(&self) -> Result<(), OperationalError> {
        ...
        if self.collected.load(std::sync::atomic::Ordering::Relaxed) {
            return Ok(());
        }
    ...
    ///
    /// Reverse edges rather than reference counts, because the edge either exists or it does not:
    /// writing one twice, or removing it twice, leaves the same set of edges, so a collection
    /// interrupted part-way can simply be run again. A count incremented twice after a crash is
    /// silently wrong for good.
    ///
    ...
    ///
    /// Re-recording an edge therefore clears a stamp a sweep had put on it. That costs the next
    /// sweep a walk it could have skipped, and never retains garbage or drops something live: an
    /// absent stamp is below every floor, so it forces the question to be settled rather than
    /// answering it. What is idempotent is the edge set, not the stamp on it.
    pub fn set_edge(&self, child: &[u8], parent: &[u8]) -> Result<(), OperationalError> {
        self.stamp_edge(child, parent, Stamp::UNKNOWN)
    }
...
pub fn open_shared(path: &Path) -> Result<Arc<MerkleStore>, OperationalError> {
    ...
    let db = rocksdb::DB::open_cf_descriptors(
        ...
        [
            ...
            // Untuned, unlike the others: what the store records about itself is a handful of
...
pub fn open_read_only(path: &Path) -> Result<Arc<MerkleStore>, OperationalError> {
    ...
    // Whatever families the store actually has, read off it rather than listed here. Opening
    // read-only takes the set exactly: name one that is absent and the open fails, leave out one
    // that is present and it is simply not opened - which turns every later use of it into the
    // panic in `refs_cf` rather than an error. Reading the set covers both a store written before
    // a family existed and one written after a later family was added.
    let names = rocksdb::DB::list_cf(&rocksdb_node_store_options(), path)
        .map_err(|error| OperationalError::OpenRocksDbFailed { error })?;

    let db = rocksdb::DB::open_cf_descriptors_read_only(
        &rocksdb_node_store_options(),
        path,
        names
            .iter()
            .map(|name| rocksdb::ColumnFamilyDescriptor::new(name, rocksdb_node_store_options())),
        false,
    )
    .map_err(|error| OperationalError::OpenRocksDbFailed { error })?;
