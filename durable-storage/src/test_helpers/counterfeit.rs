...
mod tests {
    ...
    /// We exclusively use keys with size 32 to make the size of the generated proofs predictable.
    ///
    /// This will become unnecessary once the merkle layer switches to using hashed keys.
    fn key_strategy() -> impl Strategy<Value = Key> {
        collection::vec(any::<u8>(), 32)
            .prop_map(|bs| Key::new(&bs).expect("Can convert 32 bytes to a key"))
    }
    ...
    const ZERO_DEPTH_PROOF_LENGTH: usize = 188;

    /// The extra length of proof added for each extra layer of depth.
    const PROOF_LENGTH_PER_LAYER: usize = 113;
