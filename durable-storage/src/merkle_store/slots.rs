...
pub fn slots(slots_dir: &Path) -> Result<Vec<SlotId>, OperationalError> {
    ...
    for entry in entries {
        ...
        // A slot is a directory whose name is the number written back plainly. Anything else
        // sharing the directory - a lease file, a `.pending-` leftover, a stray file - is not one.
        // Parsing alone is not enough: `u64` accepts `007`, which would put a second 7 in the list
        // and make reaping drop one more slot than it counted, taking the real one with it.
        let name = entry.file_name();
        let Some(name) = name.to_str() else {
            continue;
        };

        let Ok(slot) = name.parse::<SlotId>() else {
            continue;
        };

        if slot.to_string() != name {
            continue;
        }
...
fn reap_slot(slots_dir: &Path, slot: SlotId) -> Result<bool, OperationalError> {
    ...
    match fs::remove_dir_all(&path) {
        Ok(()) => {}
        // Already gone, which is what a repeated reap finds - and what a second process reaping
