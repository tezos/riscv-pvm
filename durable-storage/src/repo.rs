...
impl RegistryRepo for DirectoryManager {
    ...
    fn record_commit(&self, root: &CommitId) -> Result<Seq, OperationalError> {
        ...
        let mut journal = std::fs::OpenOptions::new()
            .create(true)
            .write(true)
            // Explicit because this rewrites in place: the torn tail is discarded by the
            // `set_len` below, after the new entry is written over it, not by opening.
            .truncate(false)
            .open(self.journal_file())
            .map_err(|error| OperationalError::FileWriteFailed { error })?;

        // Write at the end of the last *whole* entry rather than at the end of the file. A torn
        // write leaves a partial record that reading already ignores, but appending past it would
        // put every later entry out of alignment permanently.
        let len = journal
            .metadata()
            .map_err(|error| OperationalError::FileReadFailed { error })?
            .len();
