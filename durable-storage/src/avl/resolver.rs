...
/// Before the fold, [`drain_deletions`](Self::drain_deletions) reads all pending items from the
...
pub struct ProveResolver<R> {
    ...
    ///
    /// Populated by [`drain_deletions`](Self::drain_deletions) before the fold begins.
...
impl<R> ProveResolver<R> {
    ...
    pub fn start(inner: R, root_tree_hash: Option<Hash>) -> (Self, mpsc::Sender<ProveNodeId>) {
    ...
    ///
    /// Must be called before the fold begins so that [`deleted_node`](Self::deleted_node) returns
    /// correct results.
    #[expect(dead_code, reason = "used by MerkleLayer fold in follow-up commit")]
    ...
    Resolver<ProveNodeId, Node<ProveTreeId, Prove<'static>>> for ProveResolver<R>
{
    fn resolve<'b>(
        ...
        let resolved = self.resolve_tracking_node(&id.node)?;
    ...
    for ProveResolver<R>
{
    fn resolve<'b>(&self, id: &'b ProveTreeId) -> Result<&'b Tree<ProveNodeId>, OperationalError> {
        ...
        let resolved = self.resolve_tracking_tree(&id.tree)?;
