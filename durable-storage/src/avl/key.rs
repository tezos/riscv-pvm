...
//!   page, plus the hash of a preceding/subsequent subtree/page.
    ...
    for<'page> PageRef<'page>: LeafEncode<C>,
{
    fn fold(&self, builder: PartialHashFold<C>) -> <PartialHashFold<C> as Fold>::Folded {
        let (length, pages) = match &self.key {
            // The same fold as the tree below, over a key none of whose pages are
            // missing - so it gives the same hash.
            VerifyImpl::Full(key) => return PartialHash::Present(key_hash::<C>(key)),
...
impl PageTree {
    /// The tree of a subtree the proof carries the hash of and nothing else.
    fn blinded<C: LeafCodec>(hash: Hash) -> Self
    where
        for<'page> PageRef<'page>: LeafEncode<C>,
    {
        PageTree::Blinded {
        ...
        for<'page> PageRef<'page>: LeafEncode<C>,
    {
        match page {
            ...
            Partial::Blinded(hash) => PageTree::blinded::<C>(hash),
    ...
    for<'page> PageRef<'page>: LeafEncode<C>,
{
    ...
    match node.presence() {
        ...
        Partial::Blinded(hash) => return node.done(PageTree::blinded::<C>(hash)),
        Partial::Present(()) => {}
    }
...
///
/// This mirrors [`MinimumPresence`] variant for variant, and is deliberately kept
/// separate from it: it records what a *comparison* did, which is what accumulates
/// over the course of proof generation, and leaves the translation into what the
/// proof must carry to [`KeyAccess::presence`].
...
impl<'normal> ProveImpl<'normal> {
    ...
    fn record_ordering(&self, rhs: &Key) {
        ...
        match first_differing_page(bytes, rhs.as_ref()) {
            ...
            None if 0 < shared && shared < pages => self.record_page(shared, KeyAccess::Hashed),

            // Either the tree spans exactly the shared pages and blinds as one, or
            // there are no shared pages at all - `rhs` is empty, so the lengths order
            // the keys with nothing of this key's pages to check against it.
            None => {}
        }
    }
...
#[derive(Clone, Debug)]
enum VerifyImpl {
    /// The whole key, as the verifier built it rather than read it from a proof -
    /// so no part of it can be missing.
    Full(Key),
...
impl VerifyImpl {
    ...
    fn compare(&self, rhs: &Key) -> Comparison {
        let (length, tree) = match self {
            // Nothing of this key is missing, so it orders against `rhs` outright:
            // comparing page by page and then by length is what comparing the bytes
            // does anyway.
            VerifyImpl::Full(key) => return Comparison::Decided(key.cmp(rhs)),
