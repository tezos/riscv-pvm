...
pub fn collect<Repo: RegistryRepo>(
    ...
    // Before the prune, not after: pruning is a write, and a round asked to stop before it starts
    // has to leave the repository as it found it. Pruning first would narrow what a later round can
    // collect at - a target below the new floor is refused - so an aggressive caller that suspends
    // on every attempt would keep shrinking the collectable range while doing nothing else.
...
pub fn collect_nodes(
    ...
    if repo.merkle_read_only() {
        return Err(OperationalError::RepositoryIsReadOnly.into());
    }
    ...
    sweep::sweep(repo.merkle_store(), &roots, floor, suspend).map_err(GcError::from)
}
...
mod node_tests {
    ...
    fn a_root_between_two_positions_of_the_target_keeps_its_nodes() {
        ...
        collect_all(&fixture.repo, &target, &Suspend::new()).expect("collection should succeed");
