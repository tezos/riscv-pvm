...
fn remove_node(store: &MerkleStore, key: &[u8]) -> Result<Removed, OperationalError> {
    ...
    let children = match store.get(key) {
        ...
        Err(Error::Operational(OperationalError::NodeCollected { .. })) => Vec::new(),
        Err(Error::Operational(error)) => return Err(error),
    };

    removed.edges += children.len();
    removed.edges += store.delete_node(key, &children)?;
