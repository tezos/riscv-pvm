// SPDX-FileCopyrightText: 2026 Nomadic Labs <contact@nomadic-labs.com>
//
// SPDX-License-Identifier: MIT

//! Binary for the long-running durable storage tests.
//!
//! Exposes a `database` and a `registry` command, each with `test` and `replay`
//! subcommands. See [`octez_riscv_durable_storage::long_test`] for details.

use std::num::NonZeroUsize;
use std::path::PathBuf;
use std::time::Duration;

use anyhow::Context;
use anyhow::Result;
use clap::Args;
use clap::Parser;
use clap::Subcommand;
use octez_riscv_data::hash::Hash;
use octez_riscv_durable_storage::long_test::LongTestConfig;
use octez_riscv_durable_storage::long_test::database;
use octez_riscv_durable_storage::long_test::registry;

#[derive(Debug, Parser)]
#[command(version, long_about = None)]
struct Cli {
    #[command(subcommand)]
    command: Commands,
}

#[derive(Debug, Subcommand)]
enum Commands {
    /// Operate on the single-`Database` long test.
    Database {
        #[command(subcommand)]
        command: SubjectCommand,
    },
    /// Operate on the `Registry` long test.
    Registry {
        #[command(subcommand)]
        command: RegistryCommand,
    },
}

/// Shared arguments for a `test` run.
#[derive(Debug, Args)]
struct TestArgs {
    /// Maximum number of operations sampled per epoch.
    #[arg(long, default_value_t = 1000)]
    ops_per_epoch: usize,

    /// Number of test cases per epoch.
    #[arg(long, default_value_t = 256)]
    cases_per_epoch: u32,

    /// 32-byte hex-encoded test seed (default: run with a fresh seed, printed at startup).
    #[arg(long)]
    seed: Option<String>,

    /// Time budget in minutes (default: none).
    #[arg(long)]
    max_minutes: Option<u64>,

    /// Maximum number of epochs to run (default: run until the time budget).
    #[arg(long)]
    epochs: Option<u64>,

    /// Number of most recent epoch snapshots to keep; older snapshots and
    /// commits are cleared after each successful epoch (default: keep everything).
    #[arg(long)]
    keep_epochs: Option<NonZeroUsize>,

    /// Directory for run state and failure artifacts (default: a fresh tempdir).
    #[arg(long)]
    out_dir: Option<PathBuf>,
}

#[derive(Debug, Subcommand)]
enum SubjectCommand {
    /// Run the long test.
    Test(TestArgs),
    /// Replay the failing epoch described by `<DIR>/meta.json`.
    Replay {
        /// Failure directory containing the recorded artifacts.
        dir: PathBuf,
    },
}

#[derive(Debug, Subcommand)]
enum RegistryCommand {
    /// Run the long test.
    Test {
        #[command(flatten)]
        test: TestArgs,

        /// Number of leading permanent databases which never move, clear, or
        /// get overwritten.
        #[arg(long, default_value_t = 5)]
        permanent_databases: usize,
    },
    /// Replay the failing epoch described by `<DIR>/meta.json`.
    Replay {
        /// Failure directory containing the recorded artifacts.
        dir: PathBuf,
    },
}

/// Build a [`LongTestConfig`] from the shared test arguments.
fn config(args: TestArgs) -> Result<LongTestConfig> {
    let seed = match args.seed {
        Some(seed) => {
            let bytes = hex::decode(&seed).context("decoding hex seed")?;
            let array: [u8; Hash::DIGEST_SIZE] = bytes.as_slice().try_into().map_err(|_| {
                anyhow::anyhow!("seed must be exactly 32 bytes ({} given)", bytes.len())
            })?;
            Some(Hash::from(array))
        }
        None => None,
    };

    Ok(LongTestConfig {
        ops_per_epoch: args.ops_per_epoch,
        cases_per_epoch: args.cases_per_epoch,
        seed,
        time_budget: args.max_minutes.map(|m| Duration::from_secs(m * 60)),
        epochs: args.epochs,
        keep_epochs: args.keep_epochs,
        out_dir: args.out_dir,
    })
}

fn main() -> Result<()> {
    match Cli::parse().command {
        Commands::Database { command } => match command {
            SubjectCommand::Test(args) => database::run_long_test(config(args)?),
            SubjectCommand::Replay { dir } => database::replay_failure(&dir),
        },
        Commands::Registry { command } => match command {
            RegistryCommand::Test {
                test,
                permanent_databases,
            } => registry::run_long_test(config(test)?, permanent_databases),
            RegistryCommand::Replay { dir } => registry::replay_failure(&dir),
        },
    }
}
