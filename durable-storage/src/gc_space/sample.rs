...
impl BlobBreakdown {
    /// Entries no longer reachable from the committed root.
    pub fn dead_entries(&self) -> u64 {
        self.entries.saturating_sub(self.live_entries)
    }
...
/// once, which is what the filesystem actually spends; `apparent_bytes` is the naive sum, and the
/// gap between them is how much sharing the checkpoints are getting.
#[derive(Debug, Default, Clone, Copy)]
