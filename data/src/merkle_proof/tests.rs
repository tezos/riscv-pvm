...
impl<T: Encode> Foldable<MerkleTreeFold> for TestLeaf<T> {
    fn fold(&self, _builder: MerkleTreeFold) -> <MerkleTreeFold as Fold>::Folded {
        let data = serialise(&self.0).expect("Serialising u8 should not fail");
        MerkleTree::make_merkle_leaf(data, true)
    }
}
...
fn test_descend_tree_trailing_remainder() {
    ...
    let proof = MerkleTree::from_foldable(&seq_as_tree).compress();
    ...
    assert!(matches!(right_depth_one.children[0], MerkleProof::Leaf(_)));
    ...
    let expected: Vec<_> = (0..leaves).map(|idx| (idx, idx as u8)).collect();
