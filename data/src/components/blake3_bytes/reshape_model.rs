...
//! The BLAKE3 facts a reshaped proof rests on, checked against BLAKE3 itself rather than against
//! this crate's use of it.
//!
//! The reshape optimisation reuses chaining values computed under one length at another. That is
//! only sound because an aligned power-of-two chunk subtree has the same chaining value whatever
//! the total length is, and because those subtrees really are nodes of both canonical shapes.
//! Both are properties of BLAKE3, so they are established here directly.
...
// ----------------------------------------------------------------------------------------
// Shared helpers: the committed value hash and the canonical decomposition, written out again
// here so a bug in the component cannot hide by being made twice.
// ----------------------------------------------------------------------------------------
...
//
// The properties these facts support - that a reshaped proof round-trips, stays O(depth), and
// rejects tampering - are checked against the real component in `super::reshape_tests`. What
// stays here is the decomposition itself, which is a statement about BLAKE3 rather than about
// this crate.
// ----------------------------------------------------------------------------------------

#[test]
fn a_prefix_decomposes_into_one_subtree_per_set_bit() {
    // The reusable prefix is covered by one aligned subtree per set bit of its chunk count, which
    // is what bounds a reshape proof at one chaining value per level.
