...
impl<M: DataSpaceMode> DataSpace<M> {
    ...
    /// values will be read.
    ///
    /// The values will be read continguously from the data space. That means they are adjacent in
    ...
    ///
    /// The values will be written continguously to the data space. That means they are adjacent in
    /// the data space without any gaps. Alignment is not respected.
        ...
        let page_hash_generator = |idx| {
            ...
            match page {
                ...
                Partial::Present(data) => {
                    let hash = Hash::blake3_hash_bytes(data.contents.as_slice());
...
impl<'normal> ProveImpl<'normal> {
    ...
    fn was_accessed(&self, addr_range: Range<usize>) -> bool {
        let read_range = RangeSet2::from(addr_range.clone());
        if self.reads.borrow().intersects(&read_range) {
            return true;
        }
