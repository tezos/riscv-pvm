...
for i in $(seq 1 $NUM_ITERS); do
  nix develop --command cargo run --release --quiet --manifest-path tools/sandbox/Cargo.toml -- run --input kernels/jstz/target/riscv64gc-unknown-linux-musl/release/jstz --inbox-file "$inbox_file" --timings > $result_dir/$i.json
