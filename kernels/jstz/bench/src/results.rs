...
pub fn handle_results(
    ...
    let all_metrics = all_logs
        ...
        .map(|logs| {
            ...
            let metrics =
                check_transfer_metrics(&results, expected_transfers, exclude_warmup_transfers)?;
            check_balances(
                &results,
...
fn check_transfer_metrics(
    ...
    // The first execution is the minting call. We then have a number of warmup transfers.
    //
    // We collect the time elapsed at the _end_ of the last warmup transfer, all the way up to
    // the _end_ of the last execution (transfer).
    let duration = level.executions[total_transfers].elapsed
        - level.executions[exclude_warmup_transfers].elapsed;
