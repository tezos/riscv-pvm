...
pub use self::output::OutboxMessage;
pub use self::output::OutboxMessageError;
...
pub(crate) mod tests {
    ...
    fn write_message_level_wrap() {
        proptest!(|(level in 0u32..TEST_OUTBOX_SIZE as u32)| {
            let mut outbox = Outbox::<Normal>::default();
            let first_message = OutboxMessage::try_from(vec![1u8; 32].into_boxed_slice()).unwrap();
            let second_message = OutboxMessage::try_from(vec![2u8; 32].into_boxed_slice()).unwrap();
