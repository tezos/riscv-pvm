...
impl Unfoldable for SignalActions<Normal> {
    fn unfold<U: Unfold>(source: U) -> Result<Self, UnfoldError> {
        let mut source = source.into_node()?;

        let actions = source.next_branch()?;
        let flags = source.next_branch()?;
        let masks = source.next_branch()?;
        let restorer = source.next_branch()?;
        let thread_mask = source.next_branch()?;

        source.done(SignalActions::<Normal> {
            actions,
            flags,
            masks,
            restorer,
...
impl<C> Decode<C> for SignalActions<Normal> {
...
mod tests {
    ...
    mode_test!(test_step_into_handler, F: PvmCryptoMode, {
        type MC = M1M;
    ...
    mode_test!(test_jump_to_restorer, F: PvmCryptoMode, {
        type MC = M1M;
        type PC = EmptyPageCache;
        type DS = DurableStorageDummy;
