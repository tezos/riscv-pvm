(* This file does not have a .mli to simplify exporting the Functor for creating a RISCV backend *)

module Api = Octez_riscv_api

module MakeBackend (ProofImpl : Common.ProofSigs) = struct
  ...
  let to_api_input : Common.input -> Api.input =
   fun input ->
    match input with
  ...
  let from_api_input_request : Api.input_request -> Common.input_request =
   fun input_request ->
  ...
  module Mutable_state = struct
    ...
    let compute_step_many ?reveal_builtins:_ ?write_debug ?stop_at_snapshot:_
      ...
      | None ->
          Lwt.return (Api.octez_riscv_mut_compute_step_many max_steps state)
      | Some printer ->
          with_hooks
            printer
    ...
    let get_current_level state =
      Lwt.return (Api.octez_riscv_mut_get_level state)
    ...
    let get_reveal_request state =
      Lwt.return
        (String.of_bytes @@ Api.octez_riscv_mut_get_reveal_request state)
  ...
  let proof_start_state proof =
    riscv_hash_to_rollup_state_hash @@ ProofImpl.api_proof_start_state proof

  let proof_stop_state proof =
    riscv_hash_to_rollup_state_hash @@ ProofImpl.api_proof_stop_state proof

  let verify_proof input proof =
    let input = Option.map to_api_input input in
    let input_request = ProofImpl.api_verify_proof input proof in
    Option.map from_api_input_request input_request

  let produce_proof input state =
    ProofImpl.api_produce_proof (Option.map to_api_input input) state

  let serialise_proof proof = ProofImpl.api_serialise_proof proof

  let deserialise_proof proof = ProofImpl.api_deserialise_proof proof
