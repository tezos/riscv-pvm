...
impl JIT {
    ...
    pub fn compile_page<D, MC: MemoryConfig>(
        ...
        // does not continue with either a `Next` or `Branch` outcome, or we reach the end of the page.
        while offset < page.entries.len() {
...
mod tests {
    ...
    impl Scenario {
        fn simple(instructions: &[Instruction]) -> Self {
            Scenario {
                ...
                step_budget: ScenarioSteps::Sequence,
                ...
                // request - and therefore we expect most scenarios to run beyond the 'defined'
                // instructions, and therefore throw an exception.
                //
                // TODO: RV-840: remove this workaround
        ...
        fn run_inner(&self) {
            ...
            let max_steps = match self.step_budget {
                // TODO: RV-840: remove sequence steps workaround
                // we add an additional step by default for the exception outcome
                // - as we run an `Unknown` instruction after we finish running all the
                // instructions we specified.
                ScenarioSteps::Sequence => self.instructions.len() + 1,
                ScenarioSteps::Specific(n) => n,
            };
    ...
    impl ScenarioBuilder {
        ...
        fn set_step_budget(mut self, step_budget: usize) -> Self {
            self.step_budget = ScenarioSteps::Specific(step_budget);
            self
        }
    ...
    fn test_jr_imm() {
        ...
        let scenarios = vec![
            ...
            ScenarioBuilder::default()
                ...
                // after 2 steps we will be evaluating the li for the second time
    ...
    fn test_jalr() {
        ...
        let scenarios = vec![
            ...
            ScenarioBuilder::default()
                ...
                // after 2 steps we will be evaluating the first instruction
                .set_assert_hook(assert_hook!(|core| {
                    assert_eq!(core.hart.pc.read(), 0);
    ...
    fn test_jalr_absolute() {
        ...
        let scenarios = vec![
            ...
            ScenarioBuilder::default()
                ...
                .set_step_budget(2)
                .no_exception()
                // after 2 steps we will be evaluating the first no-op for the second time
                .set_assert_hook(assert_hook!(|core| {
                    assert_eq!(core.hart.pc.read(), 0);
    ...
    fn test_j_absolute() {
        let scenarios = vec![
            ...
            ScenarioBuilder::default()
                ...
                // *NB* absolute jump currently always exits the JIT context - hence we can't run for
                // e.g. 4 steps as we'd run the jit function twice
                .set_step_budget(2)
                .set_expected_steps(2)
                // after 2 steps we will be evaluating first instruction
                .no_exception()
                .set_assert_hook(assert_hook!(|core| {
                    assert_eq!(core.hart.pc.read(), 0);
                }))
    ...
    fn test_jit_recovers_from_compilation_failure() {
        ...
        for failure in failure_scenarios.iter() {
            ...
            // The compilation will go one past the valid nop instruction in the page, which will be an illegal instruction.
            assert_eq!(jitted_err, ExceptionCode::IllegalInstruction);
    ...
    fn test_store_float() {
        ...
        type ConstructStoreFn =
            fn(rs1: XRegister, rs2: FRegister, imm: i64, width: InstrWidth) -> I;

        const MEMORY_SIZE: u64 = M4K::TOTAL_BYTES.get() as u64;
        const FREG_VALUE: u64 = 0xFFEEDDCCBBAA9988;
        ...
        let valid_store = |constructor: ConstructStoreFn, imm: u64, expected: u64| {
            ...
            ScenarioBuilder::default()
                .set_setup_hook(setup_hook!(|core| {
                    core.hart.fregisters.write(f2, FREG_VALUE.into());
                }))
                .set_instructions(&[
                    I::new_li(NZ::x1, STORE_ADDRESS_BASE as i64, InstrWidth::Compressed),
                    constructor(x1, f2, imm as i64, InstrWidth::Uncompressed),
                    I::new_nop(InstrWidth::Compressed),
                ])
                .set_expected_steps(3)
                .set_assert_hook(assert_hook!(|core| {
        ...
        let invalid_store = |constructor: ConstructStoreFn, width: LoadStoreWidth| {
            ...
            ScenarioBuilder::default()
                .set_instructions(&[
                    I::new_li(NZ::x1, store_address_base as i64, InstrWidth::Compressed),
                    I::new_li(NZ::x2, FREG_VALUE as i64, InstrWidth::Compressed),
                    constructor(
                        x1,
                        f2,
                        store_address_offset as i64,
                ...
                // the store will fail due to being out of bounds
                .set_expected_steps(2)
                .set_expected_exception(Exception::StoreAMOAccessFault)
                .set_assert_hook(assert_hook!(|core| {
                    let value: u64 = core.main_memory.read(MEMORY_SIZE - 8).unwrap();
        ...
        let scenarios = vec![
            valid_store(I::new_f64_store, 0, FREG_VALUE),
            // invalid stores: out of bounds
            invalid_store(I::new_f64_store, LoadStoreWidth::Double),
        ];
    ...
    fn test_load_float() {
        ...
        type ConstructLoadFn = fn(rd: FRegister, rs1: XRegister, imm: i64, width: InstrWidth) -> I;
        ...
        let valid_load = |new_load: ConstructLoadFn, imm: u64, expected: u64| {
            ...
            ScenarioBuilder::default()
                ...
                .set_instructions(&[
                    ...
                    new_load(f2, x1, imm as i64, InstrWidth::Uncompressed),
                    I::new_nop(InstrWidth::Compressed),
                ])
                .set_expected_steps(3)
                .set_assert_hook(assert_hook!(|core| {
                    let value: u64 = core.hart.fregisters.read(f2).into();
                    assert_eq!(value, expected, "Found {value:x}, expected {expected:x}");
                }))
                .build()
        };
        ...
        let invalid_load = |new_load: ConstructLoadFn, width: LoadStoreWidth| {
            ...
            ScenarioBuilder::default()
                .set_instructions(&[
                    I::new_li(NZ::x1, load_address_base as i64, InstrWidth::Compressed),
                    new_load(f2, x1, load_address_offset as i64, InstrWidth::Uncompressed),
                    I::new_nop(InstrWidth::Compressed),
                ])
                // the load will fail due to being out of bounds
                .set_expected_steps(1)
        ...
        let scenarios = vec![
            // check loads - differing imm value to ensure both
            // aligned & unaligned loads are supported
            valid_load(I::new_f64_load, 8, FREG_VALUE),
            invalid_load(I::new_f64_load, LoadStoreWidth::Double),
        ];
