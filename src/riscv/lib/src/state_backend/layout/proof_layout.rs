...
use crate::state_backend::verify_backend::PartialState;
use crate::state_backend::verify_backend::Verifier;
use crate::state_backend::verify_backend::{self};
    ...
    T: Encode + Decode<()> + 'static,
{
    fn to_merkle_tree(
        ...
        let cell = Cell::<T, Ref<'_, Owned>>::bind(region.inner_region_ref());
    ...
    fn into_verifier_alloc<D: Deserialiser>(proof: D) -> VerifierAllocResult<D, Self> {
        ...
        Ok(f.map(|(data, merkle)| (Cell::from(data), merkle)))
    }
    ...
    T: Encode + Decode<()> + 'static,
{
    fn to_merkle_tree(
        ...
        let region = state.into_region();
        let access_info = region.get_access_info();
        let cells = Cells::<T, LEN, Ref<'_, Owned>>::bind(region.inner_region_ref());
    ...
    fn into_verifier_alloc<D: Deserialiser>(proof: D) -> VerifierAllocResult<D, Self> {
        ...
        Ok(proof.into_leaf::<Cells<T, LEN, Owned>>()?.map(|leaf| {
            ...
            (Cells::bind(region), merkle.into_leaf_proof_tree())
        }))
    }
