...
macro_rules! tuple_branches_proof_layout {
    ($proof:ident, $($branches:ident),+) => {
        {
            let ctx = $proof.into_node()?.map(|_unit_partial_res| ());

            let ctx = tuple_branches_proof_layout!(ctx, [], $($branches),+);
    ...
    AllocatedOf<T, Verifier>: 'static,
{
    ...
    fn to_verifier_alloc<D: Deserialiser>(proof: D) -> Result<D::Suspended<VerifierAlloc<Self>>> {
        ...
        #[expect(
            ...
            reason = "Can't use VerifierAlloc<Many<T, LEN>> since LEN computations are required 
                      and Rust has limited compile time capabilities."
        )]
