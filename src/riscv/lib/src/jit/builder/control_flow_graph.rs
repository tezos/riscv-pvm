...
use crate::jit::builder::sequence::insert_step_update_ir;
...
impl ControlFlowGraph<'_, OutcomeData, Block> {
    /// Finalise the control flow graph by inserting step counter update and budget check IR at the points
    ...
    pub fn insert_ir(
        ...
        // Collect results of analysis for step counter updates and budget checks.
        ...
        // For each outcome, insert the required IR into the function.
        for (_, outcome) in self.outcomes.iter() {
            ...
            if let Some(step_update) = outcome_data.get_step_delta() {
                insert_step_update_ir(builder, steps_remaining_var, step_update as i64);
            }
