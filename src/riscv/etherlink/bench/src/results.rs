...
fn check_transfer_metrics(txs: &[Tx]) -> Result<TransferMetrics> {
    ...
    let duration: Duration = txs.iter().map(|e| e.duration).sum();
...
impl TxBuilder {
    ...
    fn build(self, end: Duration) -> Result<Tx> {
        match self.outcome {
            Some(outcome) => Ok(Tx {
                outcome,
                duration: end - self.start,
            }),
            None => Err("Unexpected end of transaction".into()),
        }
    }
}
