...
<repo_path>/
  ...
  merkle/                               Merkle node bodies and the log of what wrote them
  merkle-commits/<n>/                   recoverable images of that store
  merkle-commits/<n>.lease              what a reader of one holds
```
...
Each node is listed in a `written` column family of the Merkle store, keyed by the commit it was last known to be held by, with the sequence number first so that ordering by key is ordering by commit.
A round reads **only the entries below its floor**: a node a retained commit wrote, or one an earlier round proved still held, sorts at or above the floor and is never looked at.

That is what keeps a round proportional to the garbage rather than to the store.
Considering every node instead costs the whole repository on every round however little there is to reclaim, which at hundreds of millions of entries is hours of work and tens of gigabytes of bookkeeping.

Whether a candidate is still held is asked from the node upwards, by walking its reverse edges.
A stamp at or above the floor ends the walk immediately: a retained root already holds the child.
Otherwise the walk climbs, and what it learns is written back onto the edges that led to a live root.
...
A candidate that turns out to be live is relisted under the newest retained root, which lifts it out of the range future rounds scan until the floor passes that root.
Candidates are read in batches and settled as they are reached, so a round holds a batch rather than a store.

<Warning>
This depends on collecting **behind** the tip.
Collect at the newest commit and the floor equals the newest retained root, so everything relisted falls below the next round's floor and the whole live set is examined again.
A rollup node collecting at the commit it has cemented has that gap; anything collecting at its own tip does not.
</Warning>
...
That rewrite costs the whole store rather than the garbage, and takes far longer than anything that would want to trigger it.

So it is a background task of its own, started explicitly and neither part of collecting nor part of taking a full commit.
A full commit is a flush and a rename; pairing it with a rewrite of the store would make the cheap operation as slow as the expensive one.
Starting a reclaim returns at once, and a second request while one is running is a no-op rather than a queued rewrite.

Reads, writes and commits carry on throughout.
The rewrite does not exclude RocksDB's own compaction either, which it would by default — holding everything else off for the length of a full-store rewrite would stall writes behind it to no purpose.

### Committing after a collection

A node is skipped at commit time when it records that it is already in the store.
After a round has deleted anything, that record is no longer evidence the body is still there, so the store takes a **new identity** and every such record stops matching.
The next commit writes what it touches instead of skipping it.

Without that, a working state whose lineage a round had dropped would skip writing nodes the round had just deleted, and commit something referring to bodies that are gone.
A read into a collected node says so; a commit over one would not, which is why this is closed by construction rather than by convention.

It costs one commit's worth of rewriting after a round that deleted something, and nothing after a round that did not.
...
Full commits are what other processes read, since the live store cannot be opened by a second writer.
A reader takes a lease on the one it opens, and reaping leaves a leased slot where it is for a later round to take.

Leases are `flock`s, so the kernel releases one held by a process that dies and a crashed reader cannot pin a slot forever.
The file a lease is taken on is created with the slot, so a reader only opens it and needs no write access to the repository.
Several readers may hold the same slot at once; only reaping needs it to itself, and it never waits for a reader.

A lease is taken on a file beside the slot rather than inside it, so that taking one does not write into a directory whose whole point is being an untouched image.
The slot is checked once the lease is held, not before, so a lease is never granted on a slot that has already gone: a reap removes the slot while holding the same lock exclusively.

## Interrupting a collection

Collection is the longest thing the storage does, and a full commit or a reap should not wait for one to finish.
A collection can be asked to stop, and both halves check often enough to stop promptly.

Stopping is safe at any point.
What was removed stays removed, each step leaves everything retained intact, and every step enumerates what is present rather than what it expected — so the next round finishes what the last one left.
A round that stopped says so, and stops before the Merkle sweep rather than sweeping against a commit set that is still half-collected.

## When a state is read after its nodes go

Collection must not run while the repository is being committed to, and two collections must not overlap.
Neither is enforced; the caller serialises them.

A reader holding a state across a collection is a different matter, and one the storage can say something useful about.
Once anything has been collected from a Merkle store, the store records it, and an absent node is then reported as collected rather than as missing commit data.
The two mean different things: missing commit data means the storage is inconsistent and there is nothing to be done, while a collected node means a state was read while the nodes behind it were reclaimed, which checking the state out again resolves.

<Note>
A store cannot know which of its absent keys it deleted, so this is reported wherever a collection has ever run.
It says a collection is the likely explanation, not a certain one.
</Note>
...
Collection runs in the caller's thread today, and stopping one is a request the caller makes and then waits on by returning.
