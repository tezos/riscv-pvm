...
<Warning>
A lease is taken on a file beside the slot, not inside it.
A reader that takes one just as a reap removes the slot can therefore be granted a lease on a slot that is no longer there, so opening it can still fail.
</Warning>
